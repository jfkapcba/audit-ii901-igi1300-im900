...
const Audit = (() => {
  ...
  function vierge(){
    return {
      ...
      meta: {
        ...
        referentiels: [], niveaux: [], filtrerNiveaux: true,
        marquage:"DR", specialFrance:false,
  ...
  function paquet(){
    return {
      type:"audit-ssi-audit", format:2, genere:new Date().toISOString(),
...
function appliquerMarquage(){
  ...
  $("#timbre-haut").textContent = regle.libelle;
  $("#timbre-bas").textContent  = regle.libelle;
  bas.hidden = !regle.timbreBas;
  $("#special-haut").hidden = !(regle.timbre && meta.specialFrance);

  document.title = (regle.timbre ? regle.libelle + " — " : "") + "Audit SSI"
...
function libelleMarquage(){
  ...
  const regle = MARQUAGES[meta.marquage] || MARQUAGES.NP;
  return regle.libelle.toUpperCase() + (meta.specialFrance ? " — SPÉCIAL FRANCE" : "");
}

/* ===========================================================================
   10. NAVIGATION ENTRE ÉCRANS
   =========================================================================== */
const ECRANS = ["accueil", "init", "audit", "synthese", "textes", "dossier"];
...
function montrerEcran(nom){
  ...
  if(nom === "textes")   EcranTextes.rendre();
...
function majContexte(){
  ...
  $("#ctx-detail").textContent =
    [meta.perimetre, refs, meta.niveaux.length ? "niveau " + meta.niveaux.join("/") : ""]
    .filter(Boolean).join(" · ");
  appliquerMarquage();
}
...
const EcranInit = {
  rendre(){
    $("#choix-referentiels").innerHTML = Catalogue.tous().map(ref => {
      ...
      return `<label class="case" data-ref="${ech(ref.id)}">
        ...
        <span><b>${ech(ref.code)} — ${ech(ref.nom)}</b>
          ...
          <small>${nb} points de contrôle${ref.niveaux ? " · niveaux : " + ref.niveaux.join(", ") : ""}${
            corpus ? " · texte officiel intégré (" + Object.keys(corpus.sections).length + " sections)" : ""}</small>
        </span></label>`;
    }).join("");

    $("#choix-niveaux").innerHTML = Object.values(NIVEAUX).map(n =>
      `<label class="case" data-niv="${n.id}">
        <input type="checkbox" name="niv" value="${n.id}">
        <span><b>${ech(n.libelle)}</b><small>${ech(n.aide)}</small></span></label>`).join("");
    ...
    $("#choix-marquage").innerHTML = Object.values(MARQUAGES).map(m =>
      `<label class="case" data-marq="${m.id}">
        ...
        <span><b>${ech(m.libelle)}</b><small>${m.timbre
  ...
  /** Met à jour l'état visuel des cases et le décompte du plan de contrôle. */
  majApercu(){
    $$("#vue-init .case").forEach(l => l.classList.toggle("coche", $("input", l).checked));
    const refs = $$("input[name=ref]:checked").map(i => i.value);
    const niveaux = $$("input[name=niv]:checked").map(i => i.value);
    const filtrer = $("#champ-filtrer-niveaux").checked;
    const apercu = $("#apercu-plan");
    if(!refs.length){ apercu.textContent = "Sélectionnez au moins un référentiel."; return; }
    if(!niveaux.length && filtrer){ apercu.textContent = "Sélectionnez au moins un niveau."; return; }
    const n = Audit.planDeControle({meta:{referentiels:refs, niveaux, filtrerNiveaux:filtrer}}).length;
  ...
  creer(evenement){
    ...
    if(!niveaux.length && $("#champ-filtrer-niveaux").checked){
      notifier("Sélectionnez au moins un niveau de protection."); return;
    }
    ...
    audit.meta.niveaux = niveaux;
    audit.meta.filtrerNiveaux = $("#champ-filtrer-niveaux").checked;
    audit.meta.marquage = ($("input[name=marq]:checked") || {value:"DR"}).value;
